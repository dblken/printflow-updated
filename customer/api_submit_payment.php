...
if ($payment_choice === 'pickup') {
    // Pay upon pick up - move straight to production
    $sql = "UPDATE orders SET 
            status = 'In Production', 
            payment_type = 'upon_pickup',
            payment_status = 'Unpaid' 
            WHERE order_id = ?";
    $success = db_execute($sql, 'i', [$order_id]);
    
    if ($success) {
        // Notify staff
        $staff_msg = "Order #{$order_id}: Customer chose Upon Pick Up. Order is now in production.";
        ...
        log_activity($customer_id, 'Payment Updated', "Chose Upon Pick Up for Order #{$order_id}");
        echo json_encode(['success' => true, 'message' => 'Choice saved. Your order is now in production!']);
        exit;
    }
} else {
